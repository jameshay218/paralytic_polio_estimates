...
#### COMMENTED: Codes in this chunk were not used.
...
for(i in 1:length(counties)){
  age[,i]<-rep(c(age_props[1,i]/5,age_props[2:17,i]/5,age_props[18,i]/15),rep(c(5,5,15),c(1,16,1))) #### Updated from age[,i]<-rep(c(age_props[1,i]/4,age_props[2:17,i]/5,age_props[18,i]/16),rep(c(4,5,16),c(1,16,1)))
}
head(age)
...
immune_stage_duration <- 1 #duration of each immune stage  #### Checked--p.252, Talbe I; it takes 4 years on average to move from the most recent stage to the last waning stage
duration_fecal <- 27.8 # susceptible/unvaccinated          #### Checked--p.254, Table II
duration_oral <- 13.4 # sim.                               #### Checked--p.254, Table II
latent_period <- 3                                         #### 3 days Checked--p.252, Table I 
prop_oral <- 0.83                                          #### Checked--p.255, Table III

vax_effect <- matrix(c(0.28,0.78,                          #### susceptibility; Checked--p.254, Table II
                       0.41,0.77,                          #### infectiousness_fecal; Checked--p.254, Table II
                       0.88,0.79,                          #### infectiousness_oral; Checked--p.254, Table II
                       0.36,0.68,                          #### shed_duration_fecal=1-17.7/27.8=0.36 for IPVx3, 1-8.9/27.8=0.68 for OPVx2; Checked--p.254, Table II
                       0.54,0.72),ncol=2,byrow=T,          #### shed_duration_oral=1-6.1/13.4=0.36 for IPVx3; 1-3.7/13.4=0.72 for OPVx2; Checked--p.254, Table II
                     dimnames=list(c("susceptibility","infectiousness_fecal","infectiousness_oral","shed_duration_fecal","shed_duration_oral"),c("IPV","OPV"))) 
# initial (first immune stage) reductions in susceptibility, infectiousness, and duration of shedding relative to unvaccinated)

vax_residual_effect <- matrix(
                     c(1e-10,0.38, #### Updated to 1e-10 from 0.02
                       0.37,0.91,
                       0.99,0.89,
                       0.42,0.87,
                       0.93,0.97),ncol=2,byrow=T,
                       dimnames=dimnames(vax_effect)) # fraction of initial vaccine effect that remains in the last immune stage))
...
immune_stage_dist<-function(t,d=immune_stage_duration,n=5){ # t = time since vaccination; d = duration of each immune stage; n = number of stages
  dist<-rep(0,n)
  s<-min(1+floor(t/d),n)
  if(s>=1){dist[s]<-1} #### Q: under what condition will S be 0 or negative? 
  return(dist)
}
...
stage_age_vax <- array(0,dim=c(5,100,2)) # dim: stage, age, IPV/OPV
...
IPV_cov_5_pre <- coverage_2*vax_profile$IPV.at.age.5.premandate #### Changed from (coverage_2 - coverage_1)*vax_profile$IPV.at.age.5.premandate # coverage with IPV at age 5 up to 2019
#### Line 158, can we account for boosting at age 5 by IPV_cov_5_pre <- coverage_2 * vax_profile$IPV.at.age.5.premandate?
IPV_cov_2019  <- coverage_3*vax_profile$IPV.in.2019 #### Changed from (coverage_3 - coverage_2)*vax_profile$IPV.in.2019 # coverage with IPV in 2019 (when religious vaccine exemptions were invalidated - applicable only to K-12) #### implicitly assume no boosting at age 5.
IPV_cov_5_post <- coverage_3*vax_profile$IPV.at.age.5.postmandate #### Changed from (coverage_3 - coverage_1)*vax_profile$IPV.at.age.5.postmandate # coverage with IPV at age 5 from 2020-onward

stage_age_vax[,,1] <- t(IPV_cov_2*t(sapply(ages-2,immune_stage_dist)) + (IPV_cov_5_pre + IPV_cov_5_post)*t(sapply(ages-5,immune_stage_dist)) + IPV_cov_2019*t(sapply(rep(3,100),immune_stage_dist))) #### Checked that 2022 is 3 years since 2019 and thus ppl should be in waning stage 4.
...
time_dist <- function(duration, weights = shedding_weights, n = 2e5){ # length(weights) = number of stages
  ...
  for(i in 1:n){
    ...
    interval <- if(t[i]>max(breaks)){k}else{min(which(t[i]<breaks))} # return the current stage i of a random person out of 4 stages
    
    if(x[i]<=weights[interval]){times<-c(times,t[i])}else{} # get a time for transmission and put it as times
  }
...
viral_conc <- function(t, age, Nab){ # age in years (carried over from peak_conc)
  ...
  y <- if(t==0){0}else{
    max(0, (peak/t)*exp( eta - 0.5*(nu^2) - ((log(t) - eta)^2)/(2*(nu + xi*log(t))^2) )) #### keep max(0,...) for there is no biological reason to give a lower bound on shedding.
  }
...
prob_infect <- function(Nab, dose){ # probability of becoming infected
  ...
  y <- 1 - (1 + dose/beta)^(-alpha*Nab^(-gamma))       #### Correspond to Eq.E p.7 of Famulare et al. suppl materials
  return(y)
}
...
transmit_time[[1]] <- rgamma(x[1],pars_fecal[1],pars_fecal[2])+rgamma(x[1],2,2/latent_period) #### 2 stages Checked--p.252, Table I 
...
#### Get prop. not protected by IPV / OPV
p_stage_age_vax <- abind((1-vax_effect_by.stage[,"IPV", "susceptibility"])*stage_age_vax[,,1],
                         (1-vax_effect_by.stage[,"OPV", "susceptibility"])*stage_age_vax[,,2],along=3)/partial # distribution of age/vaccine/stage among non-refractory vaccine recipients

```

```{r PARAMETER p_infectiousness: relative infectiousness of partially immune}

relative_duration <- 1-vax_effect_by.stage[,,c("shed_duration_fecal","shed_duration_oral")]#### 0.6 (most recent immune) :1 (fully susc)
