...
p8 <- ggplot() + 
    geom_density(data=priors,aes(x=prob_paralysis_ps,fill="Prior"),fill="black",alpha=0.1) +
    theme_classic() +
    scale_y_continuous(expand=c(0,0)) +
...
infectiousness <- function(t,latent_period=3,infect_scale,infect_shape,max_infectious_period=50){
    extraDistr::ddgamma(t-latent_period, scale=infect_scale, shape=infect_shape)/extraDistr::pdgamma(max_infectious_period, shape=infect_shape,scale=infect_scale)
}
infectiousness_ps <- function(t,latent_period=3,infect_ps_par,max_infectious_period=50){
    dexp(t-latent_period, infect_ps_par)/pexp(max_infectious_period, infect_ps_par)
}
